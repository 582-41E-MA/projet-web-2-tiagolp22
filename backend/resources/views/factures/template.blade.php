...
<body>
    ...
    <p><strong>Customer Name:</strong> {{ $commande->utilisateur->prenom ?? 'N/A' }}
        {{ $commande->utilisateur->nom ?? 'N/A' }}</p>
    ...
    <p><strong>Purchase Date:</strong> {{ \Carbon\Carbon::parse($commande->date_commande)->translatedFormat('d F Y') }}
    </p>
    <p><strong>Payment Method:</strong>
        {{ json_decode($commande->methodespaiement->nom_methode_paiement, true)['en'] ?? 'N/A' }}
    </p>
    <p><strong>Shipping Method:</strong>
        {{ json_decode($commande->methodesexpedition->nom_methode_expedition, true)['en'] ?? 'N/A' }}
    </p>
    ...
    @if ($commande->voitures->isNotEmpty())
        ...
        @foreach ($commande->voitures as $voiture)
            ...
            <p><strong>Model:</strong> {{ $voiture->modele->nom_modele }}</p>
